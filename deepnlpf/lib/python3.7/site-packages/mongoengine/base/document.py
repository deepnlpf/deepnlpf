...
class BaseDocument(object):
    ...
    def __expand_dynamic_values(self, name, value):
        ...
        if is_dict:
            value = {k: self.__expand_dynamic_values(k, v) for k, v in list(value.items())}
        else:
    ...
    def _from_son(cls, son, _auto_dereference=True, only_fields=None, created=False):
        ...
        if errors_dict:
            errors = "\n".join(
                ["Field '%s' - %s" % (k, v) for k, v in list(errors_dict.items())]
            )
    ...
    def __set_field_display(self):
        ...
        """
        fields_with_choices = [(n, f) for n, f in list(self._fields.items()) if f.choices]
