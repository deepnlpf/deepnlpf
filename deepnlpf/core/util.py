...
class PreProcessing(object):
    ...
    def __init__(self, tasks, document):
        self._tasks = tasks
        self._document = document
        
        self.nlp = StanfordCoreNLP(config.PATH['path_dir_stanford_corenlp'])
        

    def run(self):
        annotation = Boost().multithreading(self.wrapper, self._document)
        self.nlp.close()
        return annotation
    ...
    def wrapper(self, document):
        ...
        props = {
            'timeout': '1500000',
            'annotators': self._tasks,
            'pipelineLanguage': 'en',
            'outputFormat': 'json'
        }

        data = json.loads(self.nlp.annotate(document, properties=props))
